...
class IndexController extends BaseController{
    ...
    public function index(){
        ...
        //新闻动态
        $news = MS("article")->field('id,cid,title,content,thumb')->where(['cid'=>199,'is_del'=>0,'audit'=>1])->order("top desc,id desc")->limit(9)->select();
        //通知公告
        $infomation = MS("article")->field('id,cid,title,content,thumb')->where(['cid'=>200,'is_del'=>0,'audit'=>1])->order("top desc,id desc")->limit(8)->select();
        //康体服务
        $service = MS("article")->field('id,cid,title,content,thumb')->where(['cid'=>201,'is_del'=>0,'audit'=>1])->order("top desc,id desc")->limit(8)->select();
